...
public class _2CallSelector implements ContextSelector {
    ...
    public Context selectContext(CSCallSite callSite, JMethod callee) {
        Context predContext = callSite.getContext();
        int length = predContext.getLength();
        if(length == 0) {
            return ListContext.make(callSite.getCallSite());
        } else if(length == 1) {
            return ListContext.make(predContext.getElementAt(0), callSite.getCallSite());
        } else {
            return ListContext.make(predContext.getElementAt(length - 1), callSite.getCallSite());
        }
    }
