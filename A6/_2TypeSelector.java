...
public class _2TypeSelector implements ContextSelector {
    ...
    public Context selectContext(CSCallSite callSite, CSObj recv, JMethod callee) {
        Context predContext = recv.getContext();
        int length = predContext.getLength();
        if(length == 0) {
            return ListContext.make(recv.getObject().getContainerType());
        } else if(length == 1) {
            return ListContext.make(predContext.getElementAt(0), recv.getObject().getContainerType());
        } else {
            return ListContext.make(predContext.getElementAt(length - 1), recv.getObject().getContainerType());
        }
    }
